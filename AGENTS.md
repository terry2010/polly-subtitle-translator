...
- **禁止绕过业务接口造数据**：测试用户必须通过注册接口创建，禁止直接 INSERT users/user_balances 绕过 CreateUser。如必须手动造数据（如测试错误路径），注释说明原因。
- **必须验证 DB 副作用**：创建/修改操作的测试不能只验证 HTTP 响应，还要查 DB 确认数据状态正确（如注册后查 user_balances 是否有记录）。
- **关键字段断言具体值**：余额、状态、点数等业务字段必须 assert.Equal 具体值，不能只断言存在或类型正确。
- **跨模块功能必须有端到端测试**：注册→登录→翻译→扣费等跨模块链路至少有一个测试走完整流程，不能只测单个模块假设其他模块正常。
