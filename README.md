...
- 内嵌 **libmpv**（按需从 GitHub 下载，动态加载），以 WS_POPUP 悬浮窗方式叠加在主窗口播放区上方，通过 SetWinEventHook 同步位置跟随主窗口移动/缩放。
...
- **FFmpeg**：首次使用时从 BtbN/FFmpeg-Builds GitHub Releases 下载（优先 gh-proxy 加速，fallback 到 gyan.dev），安装到 `AppData\Roaming\com.zimufan.ai-subtrans\ffmpeg\`。
- **libmpv**：首次播放时从 zhongfly/mpv-winbuild GitHub Releases 下载 GPL 构建（含 D3D11/GPU 渲染器和 DXVA2/D3D11VA 硬件解码）。
...
├── docs/
│   ├── 需求文档.md                    # 完整需求规格
│   ├── auto-update-publish-guide.md   # 自动更新与发布指南
│   ├── export-dialog-plan.md          # 导出对话框设计文档
...
│       ├── player.rs                  # libmpv 下载/加载/悬浮窗叠加/播放控制
...
- FFmpeg / libmpv：无需预装，应用首次使用时自动从 GitHub 下载（国内通过 gh-proxy 加速）
...
产物位于 `C:\Users\<用户名>\.cargo-target\zimufan\release\bundle\nsis\`：
- `AI-SubTrans_<版本>_x64-setup.exe`：NSIS 安装包
- `AI-SubTrans_<版本>_x64-setup.exe.sig`：更新签名文件
...
| 组件 | 安装路径 |
| --- | --- |
| FFmpeg | `<app_data_dir>/ffmpeg/ffmpeg.exe` |
| libmpv | `<app_data_dir>/libmpv/mpv-2.dll` |

`<app_data_dir>` 为 `C:\Users\<用户名>\AppData\Roaming\com.zimufan.ai-subtrans\`。
...
当前实现为**悬浮窗叠加**方案：libmpv 以 WS_POPUP 悬浮窗方式叠加在主窗口播放区上方，通过 SetWinEventHook 监听主窗口移动/缩放事件同步悬浮窗位置，DPI 同步使用 Tauri scaleFactor。libmpv 从 zhongfly/mpv-winbuild GitHub Releases 下载 GPL 构建，含 D3D11/GPU 渲染器和 DXVA2/D3D11VA 硬件解码。

### 组件下载策略

**FFmpeg**：
- 下载源优先级：gh-proxy 加速 BtbN/FFmpeg-Builds → 直连 BtbN → gyan.dev fallback
- Windows 下载 `ffmpeg-master-latest-win64-gpl.zip`，用 `zip` crate 解压
- 安装到 `<app_data_dir>/ffmpeg/`

**libmpv**：
- 下载源：zhongfly/mpv-winbuild GitHub Releases（GPL 构建）
- 下载 `mpv-dev-x86_64-*.7z`，用 `sevenz-rust` crate 解压
...
| FFmpeg 按需下载 | ✅ | 多源加速（gh-proxy → BtbN → gyan.dev），进度显示速度/ETA |
...
| 令牌桶限流 | P2 | 翻译 QPS 精确控制（当前用 Semaphore 并发数控制） |
...
- [需求文档](./docs/需求文档.md) — 完整需求规格
- [自动更新与发布指南](./docs/auto-update-publish-guide.md) — 发布流程、故障排查、注意要点
- [导出对话框设计](./docs/export-dialog-plan.md) — 导出功能设计文档
- [错误码文档](./docs/error-codes.md) — IPC 错误码清单
- [许可证兼容性](./docs/license-compatibility.md) — GPL/LGPL 兼容性分析
- [ASS 解析器选型](./docs/ass-parser-selection.md) — ass-core/ass-editor 选型说明
