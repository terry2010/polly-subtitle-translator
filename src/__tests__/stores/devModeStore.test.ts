...
const { mockGetConfig, mockSetConfig, mockToggleDevtools } = vi.hoisted(() => ({
  mockGetConfig: vi.fn(),
  mockSetConfig: vi.fn(),
...
function resetStore() {
  useDevModeStore.setState({ devMode: false, initialized: false });
}
...
describe("devModeStore - initOnStartup", () => {
  ...
  it("重启 3 次后自动关闭 devMode", async () => {
    mockGetConfig.mockImplementation(async (key: string) => {
      if (key === "dev_mode") return "true";
      if (key === "dev_mode_restart_count") return "2";
      return null;
    });
    await getStore().initOnStartup();
    expect(getStore().devMode).toBe(false);
